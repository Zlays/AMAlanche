...
func SetupAPIRoutes(router *gin.Engine) {
	...
	api := router.Group("/api")
	{
		...
		userApi := api.Group("/user")
		{
			...
			// user
			userResourceApi := userApi.Group("/:userId/channel")
			{
				// authenticated
				...
				userResourceApi.GET("/visible", userChannelController.GetAllVisible())
				userResourceApi.GET("/owned", userChannelController.GetAllOwned())
			}
		}
	}
}
